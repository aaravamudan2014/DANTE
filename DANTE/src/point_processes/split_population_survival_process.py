...
sys.path.append(os.path.join(parent_path, 'processes'))
...
class SplitPopulationSurvival(TPP):
    ...
    def intensity(self, t, realization):
        ...
        if ('susceptible' in realization) and (realization['susceptible'] == np.int(1)):
            value = self.susceptible_intensity(t, realization)
        else:
            value = 0.0
    ...
    def susceptible_cumIntensity(self, t, realization, flag=False):
        
        if (flag) or ((self.ownName not in realization) or (t <= realization[self.ownName][0])):
    ...
    def transformEventTimes_o(self, realization):
        
        rescaled_values = []
  
        if self.ownName in realization:
            own_events = realization[self.ownName][0]
            cum = self.susceptible_cumIntensity(own_events, realization)
    ...
    def transformEventTimes(self, realization):
        ...
        if self.ownName in realization:
            ...
            cum_T = self.susceptible_cumIntensity(realization['right_censored_time'], realization, flag=True)
    ...
    def base_simulation_with_fixed_exogenous_events(self, realization, obs_t=None):
        # the exogenous events must be a source of the survival process, i.e., exogenous_name \in sourceNames
        
        susceptible_true = (realization['susceptible'] == np.int(1)) or ('susceptible' not in realization)
        if (self.ownName not in realization) and susceptible_true:
            #==================================================================
            if obs_t:
                t = obs_t
            else:
            ...
            s = t            
            ex_sources = list(set(self.sourceNames).intersection(set(realization.keys())))
            lambda_bar = max( max([ self.intensity(t+1e-9, realization) for s in ex_sources for t in realization[s] ]),\
                              self.intensity(self.mk['base'].mode(), realization) )
            
                        
            # simulation
            while s <  realization['right_censored_time']:
                u = random.uniform(0, 1)
                w = -np.log(u) / lambda_bar
                s = s + w
...
if __name__=='__main__':
    ...
    # w_true = np.concatenate((clf.coef_.flatten() , clf.intercept_.flatten()))
    
    #$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$   
    # draw susceptible lables
    susceptible_labels = np.random.binomial(1, p= sigmoid(np.dot(X_tilde, w_true)))
    susceptible_labels = np.where(susceptible_labels == 0, -1, susceptible_labels)
    
    # class
    N = 1000
    
    stop_times = np.random.randint(5, high=20, size=N).tolist()
    #N*[10]
    cids =  ['c_{}'.format(k) for k in range(N)]
    

    # TPPdata = ep.base_simulation(N, stop_times, cids)
    TPPdata = ep.base_simulation(N, stop_times, cids, add_susceptible = True, \
                    susceptible_labels = susceptible_labels, feature_vectors = X_tilde)
        
        
    pp_times = ep.transformEventTimes_TPPdata(TPPdata)
    ...
    # mk = {'base': memory_kernel.ExponentialMemoryKernel(beta=0.02)}
