...
class MultiVariateSurvivalSplitPopulation(object):
    def __init__(self, desc=None, num_users = None, num_platforms = 1, feature_vector_length = None, MemoryKernel = None):
        ...
        np.random.seed(20)
        choices = np.random.choice(self.alpha.size,5000,replace=False)
        self.alpha.ravel()[choices] = 1
        self.alpha = np.multiply(self.alpha , np.random.random((num_users, num_users)))
...
def main():
    ...
    classPrior1 = 0.2
    misclassificationProbability = 0.2
    
    num_training_realizations = 1000
