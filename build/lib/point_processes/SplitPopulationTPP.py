...
class SplitPopulationTPP(TPP):

    '''Split Population Temporal Point Process'''
    ...
    def calculate_terms(self, alpha_dash, w_dash, num_realizations, MTPPdata, dataset_dict):
        ...
        for c in range(num_realizations):
            ...
            if not isSusceptible:
                ...
                J += np.log(1 + np.exp(-np.dot(self.feature_vectors[c], w_dash))) -\
    ...
    def train(self, MTPPdata, precal_filename, validation_MTPPdata, verbose = 1):
        ...
        while True:
            ...
            A, B, D, E, F, H, J = self.calculate_terms(
            ...
            for index, alpha_val in enumerate(alpha_dash):
                updated_alpha_dash[index] = D[index]/(E[index] + B[index])
            ...
            _, _, _, _, _, updated_H, updated_J = self.calculate_terms(
            ...
            _, _, _, _, _, val_H, val_J = self.calculate_terms(updated_alpha_dash, updated_w_dash, len(
            ...
            if verbose == 1:  
                print("training likelihood:", neg_log_likelihood)
                print("validation likelihood:", validation_likelihood)
