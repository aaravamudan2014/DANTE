...
class SplitPopulationTPPDiscriminative(TPP):
    ...
    def intensity(self, t, MTPPdata):
        ...
        # print("time: ",t)
        # print("data",MTPPdata)
        # print("isSusceptible", self.isSusceptible)
        if (len(MTPPdata[0]) > 0):
    ...
    def setupTraining(self, TPPdata, feature_vectors, pre_cal_filename, validation_MTPPdata):
        """[summary]
        ...
        if not os.path.isfile("../data/"+pre_cal_filename+".pickle"):
            self._setupTrainingDone = self.precalculate_vals(
                TPPdata, pre_cal_filename, validation_MTPPdata)
        else:
            print("Pre_cal file {0} already exists, using that..".format(
                pre_cal_filename))

    def setFeatureVectors(self, feature_vectors):
        """[summary]
    ...
    def log_likelihood_generative(self, H_gen, J_gen):
        return H_gen + J_gen

    
    def calculate_terms(self, alpha_dash, w_dash, num_realizations, MTPPdata, dataset_dict):
        """[summary]
        ...
        Returns:
            [type]: [description]
        """
        A = np.zeros(len(self.w_tilde))
        D = np.zeros(len(self.sourceNames))
        E = np.zeros(len(self.sourceNames))
        B = np.zeros(len(self.sourceNames))
        F = np.zeros(len(self.w_tilde))
        E_e = np.zeros(len(self.sourceNames))
        F_e = np.zeros( (len(self.sourceNames),len(self.sourceNames)))
        ...
        for c in range(num_realizations):
            ...
            if not isSusceptible:
                A += ((1 / (1 + np.exp(-np.dot(self.feature_vectors[c], w_dash))))- (1/(np.exp(-np.dot(self.feature_vectors[c], w_dash)) + np.exp(-np.dot(alpha_dash, Psi_matrix[:, -1])))) ) *self.feature_vectors[c]

                E += (1 - (1 / (1 + np.exp(np.dot(alpha_dash, Psi_matrix[:, -1]) - np.dot(
                    self.feature_vectors[c], w_dash))))) * Psi_matrix[:, -1]
                J += np.log(1 + np.exp(-np.dot(self.feature_vectors[c], w_dash))) - np.log(np.exp(-np.dot(alpha_dash, Psi_matrix[:, -1])) + np.exp(-np.dot(self.feature_vectors[c], w_dash)))
                J_gen += np.log(1 + np.exp(-np.dot(self.feature_vectors[c], w_dash))) -\
                    np.log(np.exp(-np.dot(w_dash, self.feature_vectors[c])) + np.exp(-np.dot(
                        alpha_dash, Psi_matrix[:, -1])))
            else:
                # the first realization of its own event is when the exploit event takes place
                ...
                if np.dot(alpha_dash, Phi_matrix[:, req_index]) > 0:
                    ...
                    H +=    np.log(np.exp(-np.dot(Psi_matrix[:, req_index],alpha_dash)) + np.exp(-np.dot(self.feature_vectors[c], w_dash))) +\
                            np.dot(alpha_dash,Psi_matrix[:, req_index])  -\
                            np.log(np.dot(alpha_dash, Phi_matrix[:, req_index])) 
                    H_gen += np.log(1 + np.exp(-np.dot(self.feature_vectors[c], w_dash))) + \
                        np.dot(alpha_dash, Psi_matrix[:, req_index]) - np.log(
                            np.dot(alpha_dash, Phi_matrix[:, req_index]))
                # print(H)
                
                E_e += (1/(1 + np.exp(-np.dot(self.feature_vectors[c], w_dash) + np.dot(Psi_matrix[:, req_index],alpha_dash))))*Psi_matrix[:, req_index]
                F_e += np.outer(Psi_matrix[:, req_index], Psi_matrix[:, req_index])

                B += Psi_matrix[:, req_index]

                F += (1 / \
                    (np.exp(-np.dot(Psi_matrix[:, req_index],alpha_dash))+np.exp(-np.dot(self.feature_vectors[c],w_dash)))) * self.feature_vectors[c]

        eigVals, eigVecs = np.linalg.eig(F_e)
        lambda_max = max(eigVals)
        # print(eigVals)
        # print("Lambda_max", lambda_max)
        # input()
        # print(H, J, E_e, lambda_max)
        # input()
        return A, B, D, E, F, H, J, E_e, lambda_max, H_gen, J_gen

    def train(self, MTPPdata, precal_filename, validation_MTPPdata):
        """[summary]
        ...
        with open("../data/"+precal_filename+".pickle", 'rb') as handle:
            dataset_training_dict = pickle.load(handle)
        with open("../data/__validation__"+precal_filename+".pickle", 'rb') as handle:
            dataset_validation_dict = pickle.load(handle)

        num_realizations = len(MTPPdata)

        # init_alpha = np.array([6.27442808e-04, 8.46217235e-06, 2.25205601e-04, 3.93647154e-0])
        # init_weight = np.array([ 0.0190451,  -0.04959416, -0.04054907])


        init_alpha = np.ones(len(self.sourceNames))*10e-3
        init_weight = np.ones(len(self.feature_vectors[0]))*10e-3
        ...
        lowest_neg_log_likelihood = np.inf
        upper_bound_list = []

        multiple_restarts_list = [10e-6]
        multiple_restart_index = 0
        while True:
            iterations += 1
            A, B, D, E, F, H, J, E_e, lambda_max,H_gen, J_gen = self.calculate_terms(
                alpha_dash, w_dash, num_realizations, MTPPdata, dataset_training_dict)
            G = B + E - E_e
            updated_alpha_dash = np.zeros(len(alpha_dash))
            updated_w_dash = np.zeros(len(w_dash))
            for index, alpha_val in enumerate(alpha_dash):
                updated_alpha_dash[index] = (1/lambda_max)*(0.5 * lambda_max*alpha_dash[index] - G[index] + np.sqrt((0.5*lambda_max*alpha_dash[index] - G[index])**2  + 2 * lambda_max*D[index]))
                
            learning_rate = 0.0009

            updated_w_dash = w_dash + learning_rate * (F+A)
            neg_log_likelihood = self.log_likelihood(H_gen, J_gen)
            _, _, _, _, _, updated_H, updated_J,_,_,H_gen, J_gen= self.calculate_terms(
                updated_alpha_dash, updated_w_dash, num_realizations, MTPPdata, dataset_training_dict)
            ...
            _, _, _, _, _, val_H, val_J,_,_,val_H_gen, val_J_gen = self.calculate_terms(updated_alpha_dash, updated_w_dash, len(
                validation_MTPPdata), validation_MTPPdata, dataset_validation_dict)
            validation_likelihood = self.log_likelihood(val_H_gen, val_J_gen)
            if lowest_neg_log_likelihood > validation_likelihood:
                print("lower neg log likelihood found")
                # input()
                # save the alpha, w_tilde for the best likelihood
                # save the kernel_list as well
                best_parameter_dict = {}
                best_parameter_dict['alpha'] = alpha_dash
                best_parameter_dict['w_tilde'] = w_dash
                best_parameter_dict['neg_log_likelihood'] = validation_likelihood
                lowest_neg_log_likelihood = validation_likelihood
            print("training likeilhood:", neg_log_likelihood)
            print("validation likeilhood:", validation_likelihood)
            
            upper_bound_list.append(neg_log_likelihood)
        ...
        self.alpha = best_parameter_dict['alpha']
        self.w_tilde = best_parameter_dict['w_tilde']
        print("Best alpha value is: ", self.alpha)
        print("Best w_tilde value is: ", self.w_tilde)
        print("Best likelihood value is: ", lowest_neg_log_likelihood)

        # plt.plot(upper_bound_list)
        return TrainingStatus.CONVERGED, upper_bound_list
    ...
    def initParams(self):
        ...
        Returns:
            [type]: [description]
        """
        self.alpha = np.ones(len(self.mk))
    ...
    def generate_baseline_results(self, realizations, 
        ...
        for key, value in new_index_dict.items():
            ...
            if tc in all_tc and delta_t in all_delta:
                ...
                # feature_vectors = [x_c for x_c, x in zip(feature_vectors, selected_realizations) if x[0] ==  ]
                # selected_realizations = [x for x in selected_realizations if len(x[0]) != 1]
                # naive model
                num_exploited = 0
    ...
    def precalculate_vals(self, training_MTPPdata, pre_cal_filename, validation_MTPPdata):
        """[summary]
        ...
        def prec_dataset(MTPPdata, pre_cal_filename):
            ...
            pre_cal_filename = "../data/" + pre_cal_filename + ".pickle"
...
# function to run the discriminative model for a specific cell for comparison
def perCellLearning():
    ...
    exponential_beta = 0.0112
    power_law_beta = 0.05
    mkList = [WeibullMemoryKernel(0.5, 0.2),
    ...
    exploitProcessDisc = SplitPopulationTPPDiscriminative(
        mkList, sourceNames, stop_criteria,
        desc='Split population process with multiple kernels')
    exploitProcessGen = SplitPopulationTPP(
        mkList, sourceNames, stop_criteria,
        desc='Split population process with multiple kernels')
    
    # training for a single cell
    tc = 90
    delta_t = 150
    tup = (tc,delta_t)
    # get all relevant training and validation realizations
    training_index_list = np.load('../data/training_index_list.npy', allow_pickle=True).item()[tup]
    validation_index_list = np.load('../data/validation_index_list.npy', allow_pickle=True).item()[tup]

    training_realizations = np.array(training_realizations)[training_index_list]
    training_features = np.array(training_features)[training_index_list]
    
    validation_realizations = np.array(validation_realizations)[validation_index_list]
    validation_features = np.array(validation_features)[validation_index_list]

    scenario_name = "new_scsaenara"
    exploitProcessDisc.w_tilde = np.random.rand(3)
    exploitProcessDisc.setFeatureVectors(training_features)
    exploitProcessDisc.setupTraining(training_realizations, training_features,scenario_name , validation_MTPPdata=validation_realizations)
    _,plot_list = exploitProcessDisc.train(training_realizations, scenario_name, validation_MTPPdata= validation_realizations)
    
    # scenario_name = "new_scenssasssa"
    # exploitProcessGen.w_tilde = np.random.rand(3)
    # exploitProcessGen.setFeatureVectors(training_features)
    # exploitProcessGen.setupTraining(training_realizations, training_features,scenario_name , validation_MTPPdata=validation_realizations)
    # _,plot_list = exploitProcessGen.train(training_realizations, scenario_name, validation_MTPPdata= validation_realizations)
